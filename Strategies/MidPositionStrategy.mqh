...
public:
   // 构造函数
                     CMidPositionStrategy(double stopLoss = 80.0, double takeProfit = 160.0, int maxBars = 200);
   // 析构函数
...
//| 构造函数                                                          |
//+------------------------------------------------------------------+
CMidPositionStrategy::CMidPositionStrategy(double stopLoss = 80.0, double takeProfit = 160.0, int maxBars = 200)
  {
   m_stopLoss = stopLoss;
...
//| 析构函数                                                          |
//+------------------------------------------------------------------+
CMidPositionStrategy::~CMidPositionStrategy()
  {
   // 清理资源
  }
...
//| 开始监控进场价格                                                  |
//+------------------------------------------------------------------+
bool CMidPositionStrategy::StartEntryPriceMonitoring(double entryPrice, ENUM_TRADE_TYPE tradeType)
  {
...
//| 停止监控进场价格                                                  |
//+------------------------------------------------------------------+
void CMidPositionStrategy::StopEntryPriceMonitoring()
  {
   m_isMonitoring = false;
   m_entryPrice = 0.0;
  }
...
//| 检查是否达到进场价格                                              |
//+------------------------------------------------------------------+
bool CMidPositionStrategy::CheckEntryPriceReached(double currentPrice)
  {
...
//| 开始监控出场价格                                                  |
//+------------------------------------------------------------------+
bool CMidPositionStrategy::StartExitPriceMonitoring(double exitPrice)
  {
   if(exitPrice <= 0.0)
      return false;
...
//| 停止监控出场价格                                                  |
//+------------------------------------------------------------------+
void CMidPositionStrategy::StopExitPriceMonitoring()
  {
   m_exitPrice = 0.0;
  }
...
//| 检查是否达到出场价格(止盈/止损)                                   |
//+------------------------------------------------------------------+
bool CMidPositionStrategy::CheckExitPriceReached(double currentPrice)
  {
   if(m_exitPrice <= 0.0)
      return false;
