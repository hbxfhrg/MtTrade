...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("order_deal_reader.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("OrderDealReader")

class OrderDealReader:
    """订单和成交记录读取器"""
    ...
    def read_order_deal_data(self, file_path):
        """
    ...
    def save_orders_to_db(self, orders_df, report_file):
        """
        将订单数据保存到数据库
    ...
    def save_deals_to_db(self, deals_df, report_file):
        """
        将成交记录数据保存到数据库
    ...
    def save_to_csv(self, orders_df, deals_df, orders_csv_path="orders.csv", deals_csv_path="deals.csv"):
        """
        将订单和成交记录保存为CSV文件
    ...
    def print_summary(self, orders_df, deals_df):
        """
        打印订单和成交记录的摘要信息
        ...
        if orders_df is not None and not orders_df.empty:
            ...
            print("订单列名:", list(orders_df.columns))
            print("\n前5行订单数据:")
        ...
        if deals_df is not None and not deals_df.empty:
            print(f"成交记录总数: {len(deals_df)}")
            print("成交记录列名:", list(deals_df.columns))
            print("\n前5行成交记录数据:")
            print(deals_df.head())
        else:
            print("成交记录数据为空")
...
def main():
    """主函数"""
    # 创建读取器
    reader = OrderDealReader()
    
    # Excel文件路径
    file_path = "ReportTester.xlsx"
    report_file = "ReportTester.xlsx"
    
    # 读取订单和成交记录数据
    orders_df, deals_df = reader.read_order_deal_data(file_path)
    
    # 打印摘要信息
    reader.print_summary(orders_df, deals_df)
    
    # 保存为CSV文件
    reader.save_to_csv(orders_df, deals_df, "orders.csv", "deals.csv")
    
    # 保存到数据库
    if reader.connect_db():
        if reader.create_tables():
            orders_count = reader.save_orders_to_db(orders_df, report_file)
            deals_count = reader.save_deals_to_db(deals_df, report_file)
            logger.info(f"成功将 {orders_count} 条订单记录和 {deals_count} 条成交记录保存到数据库")
        reader.close_db()
    else:
        logger.error("无法连接到数据库，数据未保存到数据库")
